...
function renderTopAirports() {
    ...
    const rows = top.length
        ? top.map((f) => (
            `<div class="row">`
            + `<a href="#" class="icao" data-icao="${f.properties.icao}">${legendDot(TIER_COLOR_HEX[f.properties.tier] ?? TIER_COLOR_HEX[5])}${f.properties.icao}</a>`
            + `<span class="counts">${f.properties.departures} dep / ${f.properties.arrivals} arr</span>`
            + `</div>`
        )).join('')
        : '<div class="row"><span>—</span></div>';

    return `<div class="heading">Top 30 airports (8wk)</div>${rows}`;
}
